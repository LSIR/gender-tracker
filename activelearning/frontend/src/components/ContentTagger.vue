...
export default {
    data: () => ({
        ...
        // The task the user needs to perform
        //   * 0: Tag the first token of a quote or return that no quote is present
        //   * 1: Tag the last token of a quote
        //   * 2: Tag the author of a quote
        toggle_selection: 0,
    ...
    methods: {
        ...
        tagWord: function (index) {
            ...
            if (this.toggle_selection === 0){
                if (this.quote_markers[index] === 0){
                    this.quote_markers[index] = 1;
                    this.toggle_selection += 1
                }
            // Tagging the last word in the quote
            }else if (this.toggle_selection === 1){
                let tag = false;
                for (let i = 0; i <= index; ++i){
                    if (tag) {
                        this.quote_markers[i] = 1
                    }else{
                        tag = this.quote_markers[i] === 1
                    }
                }
                if (tag){
                    this.toggle_selection += 1
